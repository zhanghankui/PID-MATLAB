...
for k = 1:1:2000
    ...
    p=u_1;
    tSpan=[0 T];
    [tt,xx]=ode45('chap6_7plant',tSpan,xk,[],p);%xk [] p为初始化参数Y0
    xk=xx(length(xx),:);
    y(k)=xk(1);
    dy(k)=xk(2);
    ...
    x3(k)=f(k);
    %ESO
    epc0=z1_1-y(k);%e=z1-y
    z1(k)=z1_1+T*(z2_1-beta1*epc0);%dz1=z2-beta1*e z1=z1_1+dz1
...
xlabel('time(s)');ylabel('total uncertainies and its estimate');
